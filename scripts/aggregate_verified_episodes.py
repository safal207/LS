...
DEFAULT_CREATED_AT = "2026-06-24T01:00:00Z"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Aggregate immutable LS Verified Episodes into a review-only "
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to verified-episode.json. Repeat for multiple episodes.",
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "build/identity-learning",
    )
...
def main() -> int:
    args = parse_args()
    episodes = tuple(
        verified_episode_from_mapping(_read_json(path)) for path in args.episode
    )
    aggregation = aggregate_verified_episodes(
        ...
        metadata={
            "source": "aggregate_verified_episodes_cli",
            "input_count": len(args.episode),
        },
    )

    args.output.mkdir(parents=True, exist_ok=True)
    aggregation_path = args.output / "lesson-aggregation.json"
