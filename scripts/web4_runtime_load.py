...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run Web4 runtime load-test harness phases (phase1/phase2/phase3/phase4).",
    )
    parser.add_argument("--phase", choices=["phase1", "phase2", "phase3", "phase4"], default="phase1")
    ...
    parser.add_argument("--chaos-disconnect-rate", type=float, default=0.08)
    parser.add_argument("--burst-probability", type=float, default=0.12)
    parser.add_argument("--burst-size", type=int, default=16)
    parser.add_argument("--soak-duration-s", type=float, default=30.0)
    parser.add_argument("--target-messages-per-s", type=int, default=200)
    parser.add_argument("--gc-pressure", action="store_true")
    return parser
...
def main() -> int:
    ...
    config = LoadTestConfig(
        ...
        chaos_disconnect_rate=args.chaos_disconnect_rate,
        burst_probability=args.burst_probability,
        burst_size=args.burst_size,
        soak_duration_s=args.soak_duration_s,
        target_messages_per_s=args.target_messages_per_s,
        gc_pressure=args.gc_pressure,
    )
