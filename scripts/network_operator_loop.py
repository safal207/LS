...
def write_env_overrides(path: Path, env_overrides: dict[str, str]) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Generated by scripts/network_operator_loop.py",
        "# Safe operator overrides for graph/network policy.",
    ]
    for key in sorted(env_overrides):
        value = env_overrides[key].replace("\\", "\\\\").replace('"', '\\"')
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        nargs="?",
        const=".env.network",
        help="Write recommended env overrides to a local env file. Defaults to .env.network.",
    )
    ...
    if args.json:
        applied_env_file = None
        if args.apply_env_file:
            applied_env_file = str(write_env_overrides(Path(args.apply_env_file), actions["env_overrides"]))
        print(
            json.dumps(
                {
                    "report": report,
                    ...
                    "applied_env_file": applied_env_file,
                },
                ensure_ascii=False,
                indent=2,
            )
    ...
    if applied_env_file:
        print(f"env_file = {applied_env_file}")
    return 0
