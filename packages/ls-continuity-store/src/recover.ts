...
/** Verifies and rebuilds one subject inside an existing SQLite read snapshot. */
export function recoverSubjectFromVerifiedEvents(
...
export function recoverSubject(store: ContinuityStore, subjectId: string): ContinuationState | null {
  return store.withReadSnapshot(() =>
    recoverSubjectFromVerifiedEvents(store, subjectId)
  );
}
