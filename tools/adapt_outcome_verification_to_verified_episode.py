...
def evaluate(case: dict[str, Any]) -> dict[str, Any]:
    ovc, bindings, learning, lifecycle, episode_id, checks, faults = run(case)
    verdict, reason_code = choose(faults)
    episode = (
        build(ovc, bindings, learning, lifecycle, episode_id)
        if verdict == "WRITE_CANDIDATE"
        else None
    )
    return {
