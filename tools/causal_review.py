...
REVIEWER_KEYS = {"id", "display_name", "model"}
TARGET_KEYS = {"repository", "pr_number", "head_sha", "patch_sha256"}
...
LOCATION_KEYS = {"path", "line"}
EVIDENCE_KEYS = {"type", "reference", "excerpt"}
...
def _object(
    ...
    *,
    ...
    if extra:
        raise ContractError(f"{field} contains unknown properties: {', '.join(extra)}")
    missing = sorted((required if required is not None else allowed) - keys)
    if missing:
        raise ContractError(f"{field} is missing required properties: {', '.join(missing)}")
    return value
...
def parse_model_json(text: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise ContractError(
            "model response is not one JSON object: "
...
def validate_review(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    review = _object(raw, "review", allowed=ROOT_KEYS)
    ...
    reviewer = _object(
        review["reviewer"],
        "reviewer",
        allowed=REVIEWER_KEYS,
    )
    reviewer_id = _string(reviewer["id"], "reviewer.id")
    display_name = _string(reviewer["display_name"], "reviewer.display_name")
    model_raw = reviewer["model"]
    model = None if model_raw is None else _string(model_raw, "reviewer.model")

    target = _object(review["target"], "target", allowed=TARGET_KEYS)
    repository = _string(target["repository"], "target.repository")
    pr_number = target["pr_number"]
    if isinstance(pr_number, bool) or not isinstance(pr_number, int) or pr_number < 1:
        raise ContractError("target.pr_number must be a positive integer")
    head_sha = _string(target["head_sha"], "target.head_sha")
    if not SHA_RE.fullmatch(head_sha):
        raise ContractError("target.head_sha must be a 40-character lowercase Git SHA")
    patch_sha256 = _string(target["patch_sha256"], "target.patch_sha256")
    ...
    execution_status = _enum(
        execution["status"], "execution.status", EXECUTION_STATUSES
    )
    provenance = _enum(
    ...
    verdict_raw = review["verdict"]
    verdict = None if verdict_raw is None else _enum(verdict_raw, "verdict", VERDICTS)
    risk_level = _enum(review["risk_level"], "risk_level", RISK_LEVELS)
    findings_raw = _list(review["findings"], "findings")
    ...
    for index, raw_finding in enumerate(findings_raw):
        ...
        finding = _object(raw_finding, prefix, allowed=FINDING_KEYS)
        finding_id = _string(finding["id"], f"{prefix}.id")
        if finding_id in finding_ids:
            raise ContractError(f"duplicate finding id: {finding_id}")
        finding_ids.add(finding_id)

        severity = _enum(finding["severity"], f"{prefix}.severity", SEVERITIES)
        title = _string(finding["title"], f"{prefix}.title")
        claim_status = _enum(
        ...
        path = _string(location["path"], f"{prefix}.location.path")
        line = _optional_line(location["line"], f"{prefix}.location.line")

        causal_chain = _object(
            finding["causal_chain"],
            f"{prefix}.causal_chain",
            allowed=set(CAUSAL_KEYS),
        )
        normalized_chain = {
            key: _string(causal_chain[key], f"{prefix}.causal_chain.{key}")
            for key in CAUSAL_KEYS
        }

        evidence_raw = _list(finding["evidence"], f"{prefix}.evidence")
        if not evidence_raw:
        ...
        for evidence_index, raw_item in enumerate(evidence_raw):
            ...
            item = _object(raw_item, item_prefix, allowed=EVIDENCE_KEYS)
            evidence.append(
                {
                    "type": _enum(
                        item["type"],
    ...
    return {
        ...
        "tests_to_run": _string_list(review["tests_to_run"], "tests_to_run"),
        "human_decision_points": _string_list(
            review["human_decision_points"],
            "human_decision_points",
        ),
    }
...
def cluster_reviews(payloads: Iterable[Mapping[str, Any]]) -> dict[str, Any]:
    """Cluster evidence-bound findings for one exact frozen target."""
    ...
    if not review["findings"]:
        ...
        for finding in review["findings"]:
            ...
            for item in finding["evidence"]:
                line = f"- `{item['type']}` {item['reference']}"
                if item["excerpt"]:
                    line += f" — {item['excerpt']}"
                lines.append(line)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "validate":
            ...
            _write_json(
                args.output,
                cluster_reviews(_read_json(path) for path in args.inputs),
            )
        elif args.command == "render":
            Path(args.output).write_text(
                render_markdown(_read_json(args.input)),
                encoding="utf-8",
            )
    except (ContractError, OSError, json.JSONDecodeError) as exc:
        print(f"causal-review contract error: {exc}", file=sys.stderr)
        return 2
    return 0
