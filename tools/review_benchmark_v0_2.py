...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="LS review benchmark v0.2 provenance-aware tooling"
    )
    ...
    if args.command == "seal":
        write_json(
            args.out,
            seal_report(case, load_json(args.binding), load_json(args.report)),
        )
        return 0

    bindings = {
        "FRONTIER_MODEL": load_json(args.frontier_binding),
        "LS": load_json(args.ls_binding),
    }
    reports = {
        "FRONTIER_MODEL": load_json(args.frontier_report),
        "LS": load_json(args.ls_report),
    }
    seals = {
        "FRONTIER_MODEL": load_json(args.frontier_seal),
