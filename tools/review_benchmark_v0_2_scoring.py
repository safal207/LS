...
def score(
    ...
    if set(bindings) != LANES or set(reports) != LANES or set(seals) != LANES:
        raise BenchmarkV02Error(
            "bindings, reports, and seals must contain both v0.2 lanes"
        )
    indexes = {}
    for lane in LANES:
        validate_report(reports[lane], case, bindings[lane])
    ...
    for lane in sorted(LANES):
        ...
        if adjudication["ground_truth_complete"]:
            for truth in adjudication["known_truth"]:
                detected = any(
                    any(
                        member["lane"] == lane
                        for member in cluster_by_id[cid]["members"]
                    )
        ...
        stats["recall"] = _ratio(
            stats["known_truth_detected"],
            stats["known_truth_total"],
        )
        ...
        stats["escalation_quality"] = _ratio(
            stats["correct_escalations"],
            stats["escalation_attempts"],
        )
        stats["attribution_accuracy"] = _ratio(
            stats["attribution_correct"],
            stats["total_findings"],
        )
        stats["severity_accuracy"] = _ratio(
            stats["severity_correct"],
            stats["severity_compared"],
        )
