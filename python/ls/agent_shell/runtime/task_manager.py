...
from ls.agent_shell.runtime.state_machine import can_transition
...
class TaskManager:
    ...
    def _set_task_status(self, task_id: str, new_status: str, **extra_fields: str) -> None:
        current = self.store.fetch_one("SELECT status FROM tasks WHERE id=:task_id", task_id=task_id)
        if not current:
            raise ValueError(f"Task not found: {task_id}")
        if current["status"] != new_status and not can_transition(current["status"], new_status):
            raise ValueError(f"Invalid task transition {current['status']} -> {new_status}")
        self.store.update("tasks", "id", task_id, status=new_status, updated_at=utc_now(), **extra_fields)
    ...
    def build_plan(self, task_id: str) -> list[dict]:
        ...
        self._set_task_status(task_id, "planning")
    ...
    def plan_only(self, prompt: str, mode: str) -> tuple[Task, list[dict]]:
        ...
        self._set_task_status(task.id, "blocked", summary="Plan generated only.")
        return task, plan
    ...
    def resume_task(self, task_id: str) -> None:
        ...
        if not task:
            raise ValueError(f"Task not found: {task_id}")
        if task["status"] in {"blocked", "failed", "cancelled", "completed"}:
            raise ValueError(f"Task {task_id} is {task['status']} and cannot be resumed")
    ...
    def reject(self, task_id: str, step_id: str, reason: str) -> None:
        ...
        self._set_task_status(task_id, "blocked", summary=f"Rejected: {reason}")
