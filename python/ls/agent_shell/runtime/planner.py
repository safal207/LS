...
class Planner:
    ...
    def build_plan(self, task_id: str, prompt: str, mode: str) -> list[Step]:
        ...
        if mode == "read-only":
            skeleton = [
                (title, step_type, reason)
                for title, step_type, reason in skeleton
                if step_type not in {"write", "git", "browser", "tool", "artifact"}
            ]
            if not skeleton:
                skeleton = [
                    ("Read relevant docs", "read", "Ground the response in repository context."),
                    ("Summarize findings", "analyze", "Produce a non-mutating completion summary."),
                ]
