...
def test_export_scorecard_from_real_ledgers(tmp_path: Path) -> None:
    ...
    (quality_dir / "cycle-a.json").write_text(
        json.dumps({"cycle_id": "cycle-a", "quality_score": 0.93, "relational_field": {"relation_safety_score": 0.81}}),
        encoding="utf-8",
    )
    (quality_dir / "cycle-b.json").write_text(
        json.dumps({"cycle_id": "cycle-b", "quality_score": 0.41, "relational_field": {"relation_safety_score": 0.29}}),
        encoding="utf-8",
    )
