...
The protocol does not pretend that Gemini, Grok, Claude, or another model is interchangeable with a named Claude lane. The authoritative executor is recorded in the lane's external run binding.
...
The canonical JSON SHA-256 of this object is `run_binding_sha256`.
...
LS retains mandatory non-empty artifact nodes, relations, and probes. Frontier-model structured arrays may be empty.
...
The scorecard records each lane's executor object and provenance level. A `USER_ATTESTED` result may be compared, but must never be presented as API-verified.
