...
TRUST_ROOT_PROFILE_ID = "vtl-canonical-trust-root/v0.12"
ED25519 = "ED25519"
PUBLIC_KEY_BYTES = 32
SIGNATURE_BYTES = 64
...
def _canonical_base64_bytes(value: Any, expected_length: int) -> bool:
    decoded = _decode_base64(value)
    return decoded is not None and len(decoded) == expected_length
...
def validate_snapshot_shape(snapshot: Any) -> tuple[str, ...]:
    ...
    reasons: list[str] = []
    _add(reasons, "SNAPSHOT_FIELDS_INVALID", set(snapshot) != _SNAPSHOT_FIELDS)
    required = {
        ...
        "issued_at_ms": _nonnegative_integer,
        "not_before_ms": _nonnegative_integer,
        "not_after_ms": _nonnegative_integer,
        ...
        "signature": lambda value: _canonical_base64_bytes(value, SIGNATURE_BYTES),
    }
    ...
    if not isinstance(root, Mapping):
        ...
        _add(
            reasons,
            "SNAPSHOT_TRUST_ROOT_FIELDS_INVALID",
            set(root) != _TRUST_ROOT_FIELDS,
        )
        _add(
            reasons,
        ...
        if not isinstance(keys, list) or not keys:
            _add(reasons, "SNAPSHOT_TRUST_KEYS_INVALID", True)
        else:
            for index, key in enumerate(keys):
                if not isinstance(key, Mapping):
                    _add(reasons, f"SNAPSHOT_TRUST_KEY_INVALID:{index}", True)
                    continue
                _add(
                    reasons,
                    f"SNAPSHOT_TRUST_KEY_FIELDS_INVALID:{index}",
                    set(key) != _TRUST_ROOT_KEY_FIELDS,
                )
                specs = {
                    "signer_key_id": _non_empty_string,
                    "issuer_id": _non_empty_string,
                    "algorithm": _non_empty_string,
                    "public_key_base64": lambda value: _canonical_base64_bytes(
                ...
                for field, predicate in specs.items():
                    if field not in key or not predicate(key[field]):
                        _add(
                            reasons,
                            f"SNAPSHOT_TRUST_KEY_SCHEMA_INVALID:{index}.{field}",
                            True,
                        )
...
def validate_bootstrap_authority_shape(authority: Any) -> tuple[str, ...]:
    ...
    _add(
        ...
        set(authority) != _BOOTSTRAP_AUTHORITY_FIELDS,
    )
    _add(
        reasons,
    ...
    for index, key in enumerate(keys):
        ...
        _add(
            reasons,
            f"BOOTSTRAP_KEY_FIELDS_INVALID:{index}",
            set(key) != _BOOTSTRAP_KEY_FIELDS,
        )
        specs = {
            "bootstrap_key_id": _non_empty_string,
            "issuer_id": _non_empty_string,
...
def validate_checkpoint_shape(checkpoint: Any) -> tuple[str, ...]:
    ...
    _add(
        ...
        not _nonnegative_integer(checkpoint.get("checkpointed_at_ms")),
    )
...
def verify_canonical_trust_snapshot(
    ...
    *,
    ...
    try:
        ...
        bootstrap_authority = copy.deepcopy(bootstrap_authority)
    ...
    if key is not None:
        ...
        key_interval_valid = (
            _nonnegative_integer(key_not_before)
            and _nonnegative_integer(key_not_after)
