...
def signed_witness_payload(statement: Mapping[str, Any]) -> bytes:
    return canonical_bytes({"statement_id": statement.get("statement_id"), **witness_statement(statement)})
...
def _validate_authority(authority: Any) -> tuple[str, ...]:
    ...
    reasons: list[str] = []
    _add(reasons, "WITNESS_AUTHORITY_PROFILE_INVALID", authority.get("profile_id") != AUTHORITY_PROFILE_ID)
    _add(reasons, "WITNESS_QUORUM_CONFIG_INVALID", not _positive_integer(authority.get("quorum")))
    _add(reasons, "WITNESS_MAX_AGE_INVALID", not _positive_integer(authority.get("max_statement_age_ms")))
    algorithms = authority.get("allowed_algorithms")
    _add(
        reasons,
        "WITNESS_ALLOWED_ALGORITHMS_INVALID",
        not isinstance(algorithms, list) or not algorithms or not all(_string(value) for value in algorithms),
    )
...
def verify_witnessed_freshness(
    *,
    ...
    _add(reasons, "NOW_MS_INVALID", not _integer(now_ms))
    view_valid = (
    ...
    if not isinstance(witness_statements, Sequence) or isinstance(witness_statements, (str, bytes)) or not witness_statements:
        _add(reasons, "WITNESS_STATEMENTS_INVALID", True)

    if reasons and (not view_valid or authority_reasons or not _integer(now_ms)):
    ...
    for statement in witness_statements:
        ...
        integrity_ok = True
        profile_ok = statement["profile_id"] == STATEMENT_PROFILE_ID and statement["schema_version"] == STATEMENT_SCHEMA_VERSION
        canonical_ok = statement["canonical_profile"] == CANONICAL_PROFILE
        id_ok = statement["statement_id"] == compute_witness_statement_id(statement)
        ...
        if key is not None:
            algorithm_ok = statement["signature_algorithm"] == ED25519 and key.get("algorithm") == ED25519 and ED25519 in allowed_algorithms
            _add(reasons, "WITNESS_ALGORITHM_NOT_ALLOWED", not algorithm_ok)
            key_interval_ok = _integer(key.get("not_before_ms")) and _integer(key.get("not_after_ms")) and key["not_after_ms"] >= key["not_before_ms"]
            ...
            if algorithm_ok and key_material_ok and signature is not None:
                try:
                    Ed25519PublicKey.from_public_bytes(public_key).verify(signature, signed_witness_payload(statement))
                    signature_ok = True
                except (InvalidSignature, ValueError):
                    signature_ok = False
            _add(reasons, "WITNESS_SIGNATURE_INVALID", not signature_ok and algorithm_ok and key_material_ok)
            authority_ok = authority_ok and algorithm_ok and key_interval_ok and key_current and key.get("revoked") is False and key_material_ok
        ...
        _add(reasons, "WITNESS_SNAPSHOT_DIGEST_MISMATCH", root_match and generation_match and not digest_match)
        exact_view = root_match and generation_match and digest_match
        if signature_ok and authority_ok and fresh_ok and root_match and generation_match and not digest_match:
    ...
    return WitnessedFreshnessResult(
        ...
        accepted_witness_ids=tuple(sorted(accepted)),
        reason_codes=tuple(reasons),
    )
