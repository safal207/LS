...
The fixture contains 25 deterministic cases. CI requires Python and Node to agree on the complete `actual` result for every case, as well as exact canonical entry bytes, leaf hash, checkpoint signed bytes, checkpoint signature, checkpoint digest, and root hash.

Negative vectors include inclusion tamper, malformed path hashes, consistency tamper, tree rollback, same-size fork, peer split view, signature/key failures, stale/future checkpoints, profile/algorithm substitution, and inherited v0.13 failure.
