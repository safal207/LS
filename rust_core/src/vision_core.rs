...
impl RustFrameBuffer {
    ...
    /// Consider using `latest_frame_info()` if you only need metadata.
...
pub struct VisionPipeline {
    detector: Mutex<RustSceneChangeDetector>,
    rhythm: Mutex<RustRhythmAnalyzer>,
    running: AtomicBool,
...
impl VisionPipeline {
    ...
    pub fn new(_monitor_index: usize) -> Self {
        Self {
            detector: Mutex::new(RustSceneChangeDetector::new()),
            rhythm: Mutex::new(RustRhythmAnalyzer::new(10).expect("valid default window")),
    ...
    pub fn process_frame(
        ...
        let mut detector = self.detector.lock().map_err(|_| {
            pyo3::exceptions::PyRuntimeError::new_err("VisionPipeline detector mutex poisoned")
        })?;
        let mut rhythm = self.rhythm.lock().map_err(|_| {
            pyo3::exceptions::PyRuntimeError::new_err("VisionPipeline rhythm mutex poisoned")
        })?;

        let score =
            detector.calculate_scene_score(resized, target_w, target_h, current_ocr_text)?;
        rhythm.add_score(score);
        let activity = rhythm.classify_mode();

        {
            let mut last_activity = self.last_activity.lock().map_err(|_| {
                pyo3::exceptions::PyRuntimeError::new_err("VisionPipeline activity mutex poisoned")
            })?;
