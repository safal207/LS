...
VALIDATOR = ROOT / "validate_visual_benchmark.py"
...
class VisualBenchmarkAxisTests(unittest.TestCase):
    ...
    def assert_invalid(self, axis: Any, message: str) -> None:
    ...
    def test_non_object_axis_is_rejected(self) -> None:
        self.assert_invalid([], "visual benchmark axis must be an object")

    def test_non_object_source_item_is_rejected(self) -> None:
        axis = copy.deepcopy(self.axis)
        axis["sources"][0] = "not-an-object"
        self.assert_invalid(axis, "source\\[0\\] must be an object")

    def test_naive_timestamp_is_rejected(self) -> None:
        axis = copy.deepcopy(self.axis)
        axis["benchmarkWindow"]["observedAt"] = "2026-07-04T12:00:00"
        self.assert_invalid(axis, "must include a timezone offset")
    ...
    def test_cli_missing_axis_fails_without_traceback(self) -> None:
        result = subprocess.run(
            [sys.executable, str(VALIDATOR), str(TRAIL_FIXTURE), str(ROOT / "fixtures" / "missing-axis.json")],
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    def test_cli_invalid_json_fails_without_traceback(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            invalid_axis = Path(directory) / "invalid-axis.json"
            invalid_axis.write_text("{not json", encoding="utf-8")
            result = subprocess.run(
                [sys.executable, str(VALIDATOR), str(TRAIL_FIXTURE), str(invalid_axis)],
                capture_output=True,
                text=True,
                check=False,
            )
