...
Cognitive Trail Contract
  -> evidence snapshot, schema, generated PR-review sample, benchmark note, validator, generator, stability probe, checked-in stability sample, Route Stability Sample Contract, Route Stability Evidence Map, tests, CI workflow summary, downloadable JSON/Markdown/stability artifacts, and reviewer quickstart
...
- [Route Stability Evidence Map](ROUTE_STABILITY_EVIDENCE_MAP.md)
...
Use the reviewer-facing evidence map as a checklist:
...
```text
Check whether the full cooperative route beats the single-route baseline, participant ablations, and a bad ordering counterfactual, confirm that the checked-in sample matches the deterministic demo output for stable fields, and verify that schema, negative fixture, regression test, CI summary, CI artifact, and non-claims are tied together in one contract and evidence map.
```
...
```text
measure whether the full cooperative PR-review route beats the single-route baseline, ablation counterfactuals, and a bad ordering counterfactual; verify that the route-stability sample has a schema, negative fixture, deterministic generator, regression test, CI summary, CI artifact, and explicit non-claims; use the evidence map to identify which file proves what and which failure modes stale the evidence
```
...
This chain shows how LS turns a cooperative PR-review route into a validated artifact with a repeatability decision, a route-stability proxy, a sample contract, an evidence map, a human-readable report, and a CI-visible evidence packet.
...
```text
The ecosystem already contains connected, public, inspectable artifacts for reviewing human-plus-agent coordination, measurable cooperative routes, route-stability proxies, causal validity, trace continuity, evidence gates, action boundaries, and compute-evidence checks.
```
...
- clearer route-repeatability and route-stability evaluation across more diffs;
...
Route Stability Evidence Map
  asks which file proves what, how a reviewer verifies the route-stability chain, and which failures make the sample stale or incomplete.
