...
class GhostGPT:
    def __init__(self):
        ...
        # Initialize Access Protocol (Resonance Loop)
        self.protocol = AccessProtocol(self.window, self.audio)
        
        # Connect signals
        self.audio.text_ready.connect(self.protocol.execute_cycle)
        self.audio.status_update.connect(lambda s: setattr(self.window.lbl_status, 'setText', s))
    ...
    def set_lri_mode(self, mode):
        # Update LRI mode through Access Protocol
        if self.protocol.capu.lri.set_mode(mode):
            self.window.lbl_status.setText(f"🛡️ Mode: {mode}")
