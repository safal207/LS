...
class TestCoordinatorConsensus(unittest.TestCase):
    def test_consensus_adjustment_in_payload(self):
        ...
        self.assertIn("consensus_adjustment", result)
        self.assertGreaterEqual(result["consensus_adjustment"], -0.15)
        self.assertLessEqual(result["consensus_adjustment"], 0.15)
        self.assertEqual(result["mode"], "A")

    def test_empty_field_metrics(self):
