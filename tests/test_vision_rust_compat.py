...
def test_coordinator_fallback_to_python_on_rust_failure(monkeypatch) -> None:
    """Test that coordinator falls back to Python when Rust init fails."""
    from python.modules.perception.coordinator import VisionSubsystem
    from python.modules.perception.buffer import FrameBuffer

    # Mock Rust import failure
    def mock_import_fail(*args, **kwargs):
        raise ImportError("Mocked Rust failure")

    monkeypatch.setattr("python.modules.perception.coordinator.RustVisionPipeline", mock_import_fail, raising=False)

    coordinator = VisionSubsystem(monitor_index=1)

    # Should have Python components
    assert coordinator.pipeline is None
    assert coordinator.capturer is not None  # Python capturer
    assert isinstance(coordinator.buffer, FrameBuffer)

def test_coordinator_degraded_mode_on_both_failures(monkeypatch) -> None:
    """Test that coordinator enters degraded mode and stops when both Rust and Python fail."""
    import sys
    from python.modules.perception.coordinator import VisionSubsystem, _FallbackFrameBuffer

    # In coordinator.py, imports happen inside __init__ and _fallback_to_python_components
    # We will mock the constructor of ScreenCapturer to fail
    import python.modules.perception.capturer as cap_mod
    def mock_init_fail(*args, **kwargs):
        raise ModuleNotFoundError("Mocked failure")

    monkeypatch.setattr(cap_mod, "ScreenCapturer", mock_init_fail)

    # We also need to mock the earlier attempt in __init__
    import python.modules.perception.coordinator as coord_mod
    monkeypatch.setattr(coord_mod, "ScreenCapturer", mock_init_fail, raising=False)
    monkeypatch.setattr(coord_mod, "RustVisionPipeline", mock_init_fail, raising=False)

    coordinator = VisionSubsystem(monitor_index=1)
    # Force it into degraded state via the fallback method
    coordinator._fallback_to_python_components(1)

    # Should be in degraded mode
    assert coordinator.capturer is None
    assert isinstance(coordinator.buffer, _FallbackFrameBuffer)

    # If it starts, it should stop immediately because capturer is None
    coordinator.start()
    import time
    time.sleep(0.1)
