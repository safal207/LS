...
def test_need_market_matching_prefers_best_capability_for_high_intensity_need():
    graph = MemoryGraph()
    ...
    assert contracts[0].capability_id == "query_api"
...
def test_need_market_chain_is_recorded_in_memory_graph_and_motivation_cycle():
    ...
    capabilities = [Capability(id="run_analysis", description="analysis tool", effectiveness=0.8, cost=0.1)]
    ...
    assert ("requests", "need", "capability") in relations
    assert ("forms", "capability", "goal_contract") in relations
